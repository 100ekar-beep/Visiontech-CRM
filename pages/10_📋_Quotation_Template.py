...
if not df_items.empty:
    ...
    # Added empty string "" so that empty cells don't crash the widget
...
def quotation_dialog(quotation_data=None):
    ...
    if editor_key not in st.session_state:
        if is_new or sel_proj != saved_proj:
            st.session_state[editor_key] = pd.DataFrame([{"Item Code": "", "Description": "", "Qty": 1, "Price": 0, "Total": 0}])
        else:
            try:
    ...
    # 🌟 NEW SAFE LOGIC FOR QUOTATION WIDGET
    ...
    for idx, row in edited_items_df.iterrows():
        disp = row["Item Code"]
        if pd.notna(disp) and str(disp).strip() != "":
            disp = str(disp).strip()
            if disp in display_to_desc:
                if str(row["Description"]) != str(display_to_desc[disp]):
                    edited_items_df.at[idx, "Description"] = display_to_desc[disp]
                    edited_items_df.at[idx, "Price"] = display_to_price[disp]
                    has_changes = True
            elif disp in code_to_desc:
                if str(row["Description"]) != str(code_to_desc[disp]):
                    edited_items_df.at[idx, "Description"] = code_to_desc[disp]
                    edited_items_df.at[idx, "Price"] = code_to_price[disp]
                    has_changes = True

        qty = pd.to_numeric(edited_items_df.at[idx, "Qty"], errors='coerce')
        price = pd.to_numeric(edited_items_df.at[idx, "Price"], errors='coerce')
        tot = (0 if pd.isna(qty) else int(qty)) * (0 if pd.isna(price) else int(price))
        if edited_items_df.at[idx, "Total"] != tot:
            edited_items_df.at[idx, "Total"] = tot
            has_changes = True
    ...
    with col_add_btn:
        if st.button("➕ Add New Row", use_container_width=True, key=f"add_row_{widget_key}"):
            new_item = pd.DataFrame([{"Item Code": "", "Description": "", "Qty": 1, "Price": 0, "Total": 0}])
