...
if not df_items.empty:
    ...
    # 🌟 FIX: Include empty string in options to prevent 'None' inference crash in data_editor
...
def template_dialog(template_data=None):
    ...
    if "builder_items_df" not in st.session_state:
        if is_new:
            # Initialize with strictly empty string ""
                ...
                for str_idx, changes in edits.items():
                    ...
                    if idx < len(curr_df):
                        ...
                        if "Item Code" in changes:
                            ...
                            # Ensure we don't strip away the exact selectbox option match
