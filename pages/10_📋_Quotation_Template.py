...
if not df_items.empty:
    ...
    # 🌟 FIX: Include empty string in options to prevent 'None' inference crash
...
def template_dialog(template_data=None):
    ...
    if "builder_items_df" not in st.session_state:
        if is_new:
            # Initialize strictly with empty string
    ...
    # 🌟 NEW SAFE LOGIC (Direct dataframe binding without manual state deletion)
        ...
        if pd.notna(disp) and str(disp).strip() != "":
            ...
            if disp in display_to_desc:
                if str(row["Description"]) != str(display_to_desc[disp]):
                    edited_df.at[idx, "Description"] = display_to_desc[disp]
                    edited_df.at[idx, "Price"] = display_to_price[disp]
                    has_changes = True
            elif disp in code_to_desc:
                if str(row["Description"]) != str(code_to_desc[disp]):
                    edited_df.at[idx, "Description"] = code_to_desc[disp]
                    edited_df.at[idx, "Price"] = code_to_price[disp]
                    has_changes = True

    if has_changes:
        st.session_state.builder_items_df = edited_df
        st.rerun()
    else:
        st.session_state.builder_items_df = edited_df
    ...
    if st.button("💾 Save Template", type="primary", use_container_width=True):
        ...
        for _, r in edited_df.iterrows():
            if pd.notna(r["Item Code"]) and str(r["Item Code"]).strip() != "":
                # Background me store karte waqt exactly item code separate kar dega
