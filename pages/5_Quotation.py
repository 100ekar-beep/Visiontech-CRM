...
if not df_items.empty:
    ...
    # --- NAYI LINE: Dropdown me search ke liye Code + Description, par display me sirf Code ya structured format ---
    df_items["Display"] = df_items["Item Code"].astype(str) + " | " + df_items["Description"].astype(str)
    
    item_display_list = df_items["Display"].tolist()
    item_code_only_list = df_items["Item Code"].astype(str).tolist()
...
def template_dialog(template_data=None):
    ...
    t_name = st.text_input("QUOTATION TEMPLATE NAME *", value=def_name, key="input_t_name")
    
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<div class="modal-section-title" style="margin-top:0;">📚 Template Items</div>', unsafe_allow_html=True)

    # Session state key for holding stable rows inside popup
    rows_key = f"popup_template_rows_{def_name if not is_new else 'new'}"
    if rows_key not in st.session_state:
        if is_new:
            st.session_state[rows_key] = [{"code": item_code_only_list[0] if item_code_only_list else "", "desc": "", "price": 0}]
        else:
            try:
                ...
                for itm in items_list:
                    c = str(itm.get("Item Code", ""))
                    d = itm.get("Description", "")
                    p = int(itm.get("Price", 0))
                    loaded.append({"code": c, "desc": d, "price": p})
                st.session_state[rows_key] = loaded if loaded else [{"code": "", "desc": "", "price": 0}]
            except:
                st.session_state[rows_key] = [{"code": "", "desc": "", "price": 0}]

    # --- NAYI LINE: Stable Loop Editor (100% Prevents Popup Close Bug) ---
    with st.container():
        ...
        for i, row in enumerate(st.session_state[rows_key]):
            ...
            with c1:
                curr_code = row.get("code", "")
                # Match code to display string for selectbox
                curr_display = code_to_display.get(curr_code, item_display_list[0] if item_display_list else "")
                sel_idx = item_display_list.index(curr_display) if curr_display in item_display_list else 0
                ...
                # Extract pure item code and auto properties
                extracted_code = display_to_code.get(chosen_display, chosen_display.split(" | ")[0].strip())
                auto_desc = display_to_desc.get(chosen_display, row.get("desc", ""))
                auto_price = display_to_price.get(chosen_display, row.get("price", 0))

            with c2:
                st.text_input(f"Desc {i}", value=auto_desc, disabled=True, key=f"temp_desc_{i}", label_visibility="collapsed")

            with c3:
                entered_price = st.number_input(f"Price {i}", value=int(auto_price), step=1, key=f"temp_price_{i}", label_visibility="collapsed")

            with c4:
                if st.button("🗑️", key=f"temp_del_{i}"):
        ...
        with col_b1:
            if st.button("➕ Add Row", use_container_width=True, key="btn_add_template_row"):
                st.session_state[rows_key].append({"code": item_code_only_list[0] if item_code_only_list else "", "desc": "", "price": 0})
                st.rerun()
        with col_b2:
            if st.button("💾 Save Template", type="primary", use_container_width=True, key="btn_save_template_final"):
                if not t_name.strip():
